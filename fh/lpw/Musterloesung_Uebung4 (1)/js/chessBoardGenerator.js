...
function createChessBoard() {
    ...
    //festlegen des Ausgabebereichs in einer Variable
    let boardView = document.getElementById('chessBoard-view');
    //entfernen möglicher vorhandener Inhalte
    boardView.innerHTML = "";

    //vorbereiten der Daten, auf denen das Feld basiert
    //gekapselt in einem Daten Objekt
    let labelsData = {
        "rowLabels": [1, 2, 3, 4, 5, 6, 7, 8],
        "colLabels": ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
    };

    //aufruf zum Ausführen um das Schachbrett zu zeichnen
...
function getFieldsRow(fieldCount, label, rowIsEven) {
    ...
    let sideLabel = getSideLabel(label, true);
